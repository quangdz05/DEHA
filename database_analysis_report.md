...
Hệ thống bao gồm 15 thực thể (bảng dữ liệu) chính được phân nhóm theo nghiệp vụ:
...
#### 5. Bảng `MembershipPackage` (Khung gói tập hội viên)
...
#### 6. Bảng `UserMembership` (Thẻ hội viên của người dùng)
...
#### 7. Bảng `MembershipFreeze` (Lịch sử bảo lưu thẻ hội viên)
...
#### 8. Bảng `TrainerSchedule` (Khung thời gian rảnh của PT)
...
#### 9. Bảng `UserPTPackage` (Đăng ký gói PT của người dùng)
...
| `selected_weekdays`| JSONField | Ví dụ: `[0,2,4]` (Thứ 2, 4, 6) | Các thứ cố định trong tuần |
| `start_time` | TimeField | | Khung giờ tập cố định |
| `end_time` | TimeField | | Khung giờ kết thúc cố định |
| `created_at` | DateTimeField | `auto_now_add=True` | Thời điểm tạo |
| `updated_at` | DateTimeField | `auto_now=True` | Thời điểm cập nhật |

#### 10. Bảng `PTBooking` (Lịch tập chi tiết của gói PT)
...
#### 12. Bảng `InvoiceItem` (Chi tiết mục hóa đơn)
...
#### 13. Bảng `Payment` (Thanh toán giao dịch)
...
#### 14. Bảng `Review` (Đánh giá & Phản hồi)
...
#### 15. Bảng `EmailSetting` (Cấu hình Email SMTP)
...
Dưới đây là sơ đồ Mermaid ERD mô tả các mối liên kết thực tế của 15 bảng dữ liệu hiện tại trong hệ thống Gym Booking:
...
## 4. Các Thay Đổi Dọn Dẹp CSDL Gần Đây (Recent Database Cleanup Changes)
Để tối ưu hóa cấu trúc cơ sở dữ liệu vật lý và dọn dẹp các tính năng dư thừa/trùng lặp theo nghiệp vụ mới:
1. **Xóa bỏ bảng `PTPackage`**: Hệ thống đã bỏ luồng bán gói PT đóng sẵn, chuyển sang tính giá động buổi tập HLV 1-1 theo tháng (`số tháng tập * số buổi phát sinh * giá 1-1 của HLV`). Bảng `PTPackage` đã được DROP hoàn toàn khỏi DB.
2. **Loại bỏ cột `package` trong `UserPTPackage`**: Trường khóa ngoại này đã được xóa vì `UserPTPackage` giờ được tính toán động dựa vào thời gian tập thực tế của học viên.
3. **Xóa bỏ bảng `TrainerMonthlyBooking`**: Để loại bỏ việc trùng lặp tính năng giữa luồng đăng ký thô (chưa sinh lịch) và luồng tự động sinh lịch tập chi tiết (`PTBooking`), bảng `TrainerMonthlyBooking` đã được DROP hoàn toàn khỏi DB. Luồng đăng ký PT theo tháng giờ chỉ sử dụng `UserPTPackage` và `PTBooking` để sinh lịch tập tự động và kiểm tra trùng lịch.